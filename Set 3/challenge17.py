...
def cbc_attacker_interface(ciphertext):
    ...
    for i in range(ct_length-1, -1, -1):
        ...
        desired_padding = 16 - i % 16
        for byte_int in range(255):
            print(byte_int)
            byte = bytes([byte_int])
            print(xor_block)
            xor_block = xor_block[:-desired_padding] + byte*desired_padding
            print(xor_block)
            print(ciphertext)
            ciphertext = ciphertext[:block_start-16] + xor_block + \
                ciphertext[block_start:block_start+16] if block_start > 0 \
                else ciphertext[block_start+block_start+16]
            print(ciphertext)
            print(ciphertext[:block_start])
            if block_start > 0:
                if cbc_padding_oracle(ciphertext, random_iv):
                    break
                else:
                    print(False)
            else:
                if cbc_padding_oracle(ciphertext, xor_block):
                    break
                else:
                    print(False)
        plaintext += bytes([xor_block[position_in_block] ^ 1])
        print(plaintext)
        break

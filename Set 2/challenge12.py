...
def detect_oracle_block_size(oracle_function):
    ...
    while True:
        ...
        if new_ciphertext_size != ciphertext_size: return new_ciphertext_size - ciphertext_size
        else: ciphertext_size = new_ciphertext_size; continue
...
def byte_at_a_time_ecb_decryption(oracle_function):
    ...
    assert len(oracle_function(b'')) % block_size == 0, "Oracle function must produce a ciphertext that is a multiple of block size"
    plaintext_length = len(oracle_function(b''))

...
def encrypt_aes_cbc_mode(plaintext, key, initialization_vector):
    assert len(initialization_vector) == 16, "Initialization vector is not 128 bits"
    ...
    for i in range(len(plaintext_blocks)):
        if i == 0: plaintext_blocks[0] = set1.fixed_xor(initialization_vector, plaintext_blocks[0])
        else: plaintext_blocks[i] = set1.fixed_xor(ciphertext_blocks[i-1], plaintext_blocks[i])
        ciphertext_blocks[i] = set1.encrypt_aes_ecb_mode(plaintext_blocks[i], key)
...
def decrypt_aes_cbc_mode(ciphertext, key, initialization_vector):
    ...
    for i in range(len(ciphertext_blocks)):
        plaintext_blocks[i] = set1.decrypt_aes_ecb_mode(ciphertext_blocks[i], key)
        if i == 0: plaintext_blocks[0] = set1.fixed_xor(plaintext_blocks[0], initialization_vector)
        else: plaintext_blocks[i] = set1.fixed_xor(plaintext_blocks[i], ciphertext_blocks[i-1])      
...
# plaintext_decrypt = decrypt_aes_cbc_mode(ciphertext, bytes(key, "utf8"), initialization_vector)
# print("Decrypted ciphertext:")
# print(plaintext_decrypt.decode("utf8"))

# plaintext = "Hello my name is Thomas Kim and I am an aspiring cryptographer."
# ciphertext = encrypt_aes_cbc_mode(bytes(plaintext, "utf8"), bytes(key, "utf8"), initialization_vector)
# plaintext_decrypt = decrypt_aes_cbc_mode(ciphertext, bytes(key, "utf8"), initialization_vector)
